    ...
    return (
        <div className="flex flex-col h-[600px] border border-border/50 bg-background/30 rounded-xl overflow-hidden shadow-2xl relative">
            <div className="bg-surface/90 border-b border-border/50 p-4 shrink-0 flex items-center justify-between">
                <h3 className="font-mono text-sm uppercase tracking-wider text-text-secondary opacity-90">Neural_Comms_Link</h3>
                <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-accent animate-pulse"></span>
                    <span className="font-mono text-xs uppercase tracking-wide text-accent">Active_Session</span>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] bg-blend-soft-light relative">
                {messages.length === 0 && currentState !== 'PROCESSING' && currentState !== 'LISTENING' && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center opacity-30 select-none pointer-events-none text-center p-8">
                        ...
                        <p className="font-mono text-xs mt-2 text-text-secondary opacity-80 max-w-xs leading-relaxed uppercase tracking-wide">
                ...
                {messages.map((msg, i) => {
                    ...
                    return (
                        <div key={msg.id} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'} max-w-full`}>
                            {isFirstOrSeparated && (
                                <div className={`mb-2 font-mono text-xs uppercase tracking-wider opacity-80 flex items-center gap-2 ${isUser ? 'flex-row-reverse' : ''}`}>
                                    {!isUser && msg.agent && (
               ...
               `} style={!isUser && msg.agent ? { color: msg.agent.color } : {}}>
                ...
                {currentState === 'PROCESSING' && (
                    <div className="flex flex-col items-start max-w-full">
                        <div className="mb-2 font-mono text-xs uppercase tracking-wider opacity-80">System_Processing</div>
                ...
                {currentState === 'LISTENING' && (
                    <div className="flex flex-col items-end max-w-full">
                        <div className="mb-2 font-mono text-xs uppercase tracking-wider text-secondary opacity-80">User_Capturing</div>
                    </div>
                )}
